...
TRAIN_CONFIG = DiffusionTrainConfig(
    ...
    epochs=4,  # 训练轮数（时间紧的简化值；预训练底座上少量 epoch 即可）。 
    max_train_steps=None,  # 固定总步数时填整数；None 表示按 epochs 自动推算。
    learning_rate=1e-4,  # 学习率（已关 scale_lr，直接生效，不再乘 batch）。
    scale_lr=False,  # 时间紧用固定 LR，关闭按 batch 放大以免 LR 过高不稳。
    ...
    max_train_samples=8000,  # 时间紧的训练子集（dataset 内已做确定性 shuffle 保持 task_type 比例）。
